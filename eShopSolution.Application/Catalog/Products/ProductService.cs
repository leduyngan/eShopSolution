...
using eShopSolution.Utilityes.Exceptions;
using System;
...
{
    public class ProductService : IProductServie
    {
        ...
        public async Task<string> SaveFile(IFormFile file)
        {
            ...
            string exName = Path.GetExtension(originalFileName);
            var fileName = $"{Guid.NewGuid()}{exName}";
            await _storageService.SaveFileAsync(file.OpenReadStream(), fileName);
            return "/" + USER_CONTENT_FOLDER_NAME + "/" + fileName;
        }
        public async Task<string> SaveFile1(IFormFile file)
        {
            var originalFileName = ContentDispositionHeaderValue.Parse(file.ContentDisposition).FileName.Trim('"');
            string exName = Path.GetExtension(originalFileName);
            var fileName = $"{Guid.NewGuid()}{exName}";
            await _storageService.SaveFileAsync1(file, fileName);
            return "/" + USER_CONTENT_FOLDER_NAME + "/" + fileName;
        }
        ...
        public async Task<ProductImageViewModel> GetImageById(int imageId)
        {
            ...
            if (image == null)
            {
                throw new EShopException($"Cannot find an image with id{image}");
            }
        ...
        {
            var product = await _context.Products.FindAsync(productId);
            var productTranslaion = await _context.ProductTranslations.FirstOrDefaultAsync(x => x.ProductId == productId && x.LanguageId == languageId);
            ...
            var productViewModel = new ProductVm()
            {
                ...
                Description = productTranslaion != null ? productTranslaion.Description : null,
                LanguageId = productTranslaion != null ? productTranslaion.LanguageId : null,
                Details = productTranslaion != null ? productTranslaion.Details : null,
                Name = productTranslaion != null ? productTranslaion.Name : null,
                OriginalPrice = product.OriginalPrice,
                Price = product.Price,
                SeoAlias = productTranslaion != null ? productTranslaion.SeoAlias : null,
                SeoDescription = productTranslaion != null ? productTranslaion.SeoDescription : null,
                SeoTitle = productTranslaion != null ? productTranslaion.SeoTitle : null,
                Stock = product.Stock,
